...
public class GLUtils {
    ...
    public static int buildProgram(String vertexShader, String fragmentShader) {
        ...
        if (linkStatus[0] == 0) {
            //链接失败
            Log.d("test",glGetProgramInfoLog(programId));
    ...
    private static int compileShader(int type, String shaderCode) {
        ...
        if (shaderObjectId == 0) {
            Log.e("test","『ShaderUtil-compileShader』创建Shader失败");
            return 0;
        }
        ...
        Log.d("test","Results of compiling source" + "\n:" + glGetShaderInfoLog(shaderObjectId));
        if (compileResult[0] == 0) {
            glDeleteShader(shaderObjectId);
            Log.d("test","【ShaderUtil-compileShader】shader编译失败");
            return 0;
        }
